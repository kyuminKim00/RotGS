...
renderCUDA(
	...
	float* __restrict__ weight_per_gaussian
	)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float color_weight_per_gaussian = 0;
			// Eq. (3) from 3D Gaussian splatting paper.
			for (int ch = 0; ch < CHANNELS; ch++)
			{
				C[ch] += features[collected_id[j] * CHANNELS + ch] * alpha * T;
				color_weight_per_gaussian += features[collected_id[j] * CHANNELS + ch] * alpha * T;
			}
			weight += alpha * T;
			// weight_per_gaussian = weight;
			atomicAdd(&weight_per_gaussian[collected_id[j]], color_weight_per_gaussian);
